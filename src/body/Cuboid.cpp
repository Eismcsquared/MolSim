    ...
    for (unsigned int i = 0; i < N[0]; ++i) {
        for (unsigned int j = 0; j < N[1]; ++j) {
            for (unsigned int k = 0; k < N[2]; ++k) {
                ...
                std::array<double, 3> vBrownian = maxwellBoltzmannDistributedVelocity(avgVelocityBrownian, dimensionBrownian);
...
Cuboid::Cuboid(const std::array<double, 3> &x, const std::array<double, 3> &v, const std::array<unsigned int, 3> &n,
               double m, double distance, double avgVelocityBrownian, int dimensionBrownian) : x(x), v(v), N(n), m(m), distance(distance),
                                                                        avgVelocityBrownian(avgVelocityBrownian), dimensionBrownian(dimensionBrownian) {}
