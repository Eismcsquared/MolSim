...
Cuboid::Cuboid(const std::array<double, 3> &x, const std::array<double, 3> &v, const std::array<unsigned int, 3> &n,
               ...
               Cluster(x, v, m, distance, avgVelocityBrownian, dimension), N(n), stationary(false) {}

Cuboid::Cuboid(const std::array<double, 3> &x, const std::array<double, 3> &v, const std::array<unsigned int, 3> &n,
               double m, double distance, double avgVelocityBrownian, int dimension, int type, double epsilon, double sigma,
               bool stationary):
        Cluster(x, v, m, distance, avgVelocityBrownian, dimension, type, epsilon, sigma), N(n), stationary(stationary){}
    ...
    for (unsigned int i = 0; i < N[0]; ++i) {
        for (unsigned int j = 0; j < N[1]; ++j) {
            for (unsigned int k = 0; k < N[2]; ++k) {
                ...
                particles.emplace_back(x_particle, v_particle, m, type, epsilon, sigma, stationary);
            }
        }
    }
}
